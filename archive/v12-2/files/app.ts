...
function onStopClick(e: Event) {
  ...
  sounds["click"]?.play();
  if (btnStopEl) {
    gsap.to(btnStopEl, { scale: 0.9, yoyo: true, repeat: 1, duration: 0.1 });
  }
  timeUp(true);
}
  ...
  if (slotCount === 3) {
    if (btnPlusEl && btnMinusEl && sumDisplayEl && ctrlPanelEl) {
      ...
      if (diff <= 1) {
        if (!isShaking) {
          isShaking = true;
          gsap.to(sumDisplayEl, { rotation: 2, yoyo: true, repeat: -1, duration: 0.05 });
        }
      } else {
        if (isShaking) {
          isShaking = false;
          gsap.killTweensOf(sumDisplayEl);
          gsap.set(sumDisplayEl, { rotation: 0 });
        }
      }
    }
  } else {
