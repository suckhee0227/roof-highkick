...
function showRanking(isFinal) {
  ...
  if (isFinal) {
    ...
    confetti_module_default({
      ...
      origin: { y: 0.3 },
      zIndex: 9999,
      colors: ["#FFD700", "#99FF00", "#0080FF", "#FF4040"]
    });
  }
}
function clearUI() {
...
function onStartClick() {
  ...
  if (btn) {
    ...
    gsapWithCSS.fromTo(btn, { scale: 1 }, { scale: 1.1, duration: 0.15, yoyo: true, repeat: 1, onComplete: showTurnPopup });
  }
}
function showTurnPopup() {
  clearUI();
  let text = `${playerNamesL[currentRoundIndex]} ${textData.vsLabel} ${playerNamesR[currentRoundIndex]}
...
function setupPlayUI() {
  ...
  let nameL = AppHelper.createUIElement(
    ...
    playerNamesL[currentRoundIndex]
  );
  ...
  let nameR = AppHelper.createUIElement(
    ...
    "",
    { fontSize: "50px", color: "#FFF", fontWeight: "bold", fontFamily: "sans-serif" },
    playerNamesR[currentRoundIndex]
  );
...
function showFloatingText(val, x, y) {
  ...
  gsapWithCSS.to(txt, { y: "-=150", opacity: 0, duration: 1, ease: "power1.out", onComplete: () => txt.remove() });
}
  ...
  if (!leftStopped) {
    if (countL === 3) {
      ...
      if (Math.abs(totalL - appData.targetScore) <= 1 && gameState === "PLAY") {
        if (!isShakingL) {
          isShakingL = true;
          gsapWithCSS.fromTo(shakeOffsetL, { x: -8 }, { x: 8, yoyo: true, repeat: -1, duration: 0.05 });
        }
      } else {
        isShakingL = false;
  ...
  if (!rightStopped) {
    if (countR === 3) {
      ...
      if (Math.abs(totalR - appData.targetScore) <= 1 && gameState === "PLAY") {
        if (!isShakingR) {
          isShakingR = true;
          gsapWithCSS.fromTo(shakeOffsetR, { x: -8 }, { x: 8, yoyo: true, repeat: -1, duration: 0.05 });
        }
      } else {
        isShakingR = false;
...
function timeUp(isStopAll = false) {
  ...
  players.push({
    name: playerNamesL[currentRoundIndex],
    score: sumL,
  ...
  players.push({
    name: playerNamesR[currentRoundIndex],
    score: sumR,
  ...
  gsapWithCSS.from(tUp, { scale: 0, rotation: 10, duration: 1, ease: "elastic.out(1, 0.4)" });
  setTimeout(() => {
    tUp.remove();
...
function render3() {
  ...
  if (gameState === "PLAY" || gameState === "TIMEUP") {
    ...
    drawSlotBox(480, 850, slotCountL === 3, shakeOffsetL.x);
    drawSlotBox(1440, 850, slotCountR === 3, shakeOffsetR.x);
    SLOTS_L.forEach((s) => {
      ctx.strokeStyle = "#444";
      ctx.lineWidth = 10;
      ctx.beginPath();
