    ...
    function fireConfettiCycle() {
      ...
      (function frame() {
        ...
        if (Date.now() < end) {
          ...
          setTimeout(fireConfettiCycle, 2000);
        }
      })();
    }
...
function renderPlayerInputs() {
  ...
  for (let i = 0; i < selectedRoundCount; i++) {
    ...
    (inpL as HTMLInputElement).value = i < savedL.length ? savedL[i] : `L${i + 1}`;
    ...
    (inpR as HTMLInputElement).value = i < savedR.length ? savedR[i] : `R${i + 1}`;
...
function showFloatingText(val: number, x: number, y: number) {
  ...
  gsap.to(txt, { y: targetY, opacity: 0, duration: 1, ease: "power1.out", onComplete: () => txt.remove() });
}
  ...
  if (!leftStopped) {
    if (countL === 3) {
      ...
      if (Math.abs(totalL - appData.targetScore) <= appData.shakeTolerance && gameState === "PLAY") {
        if (!isShakingL) {
          isShakingL = true;
          gsap.fromTo(shakeOffsetL, { x: -2.6 }, { x: 2.6, yoyo: true, repeat: -1, duration: 0.05 });
        }
      } else {
        isShakingL = false;
  ...
  if (!rightStopped) {
    if (countR === 3) {
      ...
      if (Math.abs(totalR - appData.targetScore) <= appData.shakeTolerance && gameState === "PLAY") {
        if (!isShakingR) {
          isShakingR = true;
          gsap.fromTo(shakeOffsetR, { x: -2.6 }, { x: 2.6, yoyo: true, repeat: -1, duration: 0.05 });
        }
      } else {
        isShakingR = false;
...
function timeUp(isStopAll: boolean = false) {
  ...
  gsap.fromTo(tUp, { scale: 0, opacity: 0 }, { scale: 1, opacity: 1, duration: 0.6, ease: "back.out(1.5)" });
