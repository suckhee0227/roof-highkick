...
function setupIntroUI() {
  ...
  (nameInp as HTMLInputElement).placeholder = textData.namePlaceholder;
  ...
  (numInp as HTMLInputElement).placeholder = textData.numPlaceholder;
...
function onStartClick() {
  ...
  let nameVal = (document.getElementById("nameInput") as HTMLInputElement).value.trim();
  let numVal = (document.getElementById("numInput") as HTMLInputElement).value.trim();
  ...
  if (btn) {
    gsap.to(btn, { scale: 1.1, duration: 0.15, yoyo: true, repeat: 1, onComplete: showGoPopup });
  }
}
