...
function setupIntroUI() {
  ...
  let nameInp = AppHelper.createUIElement("input", "nameInput", {
    width: "100%",
    height: "120px",
    fontSize: "50px",
    textAlign: "center",
  ...
  nameInp.placeholder = textData.namePlaceholder;
  let numInp = AppHelper.createUIElement("input", "numInput", {
    width: "100%",
    height: "120px",
    fontSize: "50px",
    textAlign: "center",
  ...
  numInp.placeholder = textData.numPlaceholder;
  ...
  introForm.appendChild(nameInp);
  introForm.appendChild(numInp);
...
function onStartClick() {
  ...
  let numVal = document.getElementById("numInput").value.trim();
  currentPlayerName = nameVal || "Guest";
  currentPlayerNum = numVal || "0000";
  let btn = document.getElementById("startBtn");
  if (btn) {
    gsapWithCSS.to(btn, { scale: 1.1, duration: 0.15, yoyo: true, repeat: 1, onComplete: showGoPopup });
  }
}
  ...
  if (slotCount === 3) {
    if (btnPlusEl && btnMinusEl && sumDisplayEl && ctrlPanelEl) {
      ...
      if (diff <= 1) {
        if (!isShaking) {
          isShaking = true;
          gsapWithCSS.to(sumDisplayEl, { x: "+=10", yoyo: true, repeat: -1, duration: 0.05 });
        }
      } else {
        if (isShaking) {
          isShaking = false;
          gsapWithCSS.killTweensOf(sumDisplayEl);
          gsapWithCSS.set(sumDisplayEl, { x: 0 });
        }
      }
    }
  } else {
...
function onMinusClick(e) {
  ...
  sounds["click"]?.play();
  if (btnMinusEl) {
    gsapWithCSS.to(btnMinusEl, { scale: 0.9, yoyo: true, repeat: 1, duration: 0.1 });
...
function onPlusClick(e) {
  ...
  sounds["click"]?.play();
  if (btnPlusEl) {
    gsapWithCSS.to(btnPlusEl, { scale: 0.9, yoyo: true, repeat: 1, duration: 0.1 });
...
var import_howler, Ball, Particle, appData, textData, assetList, appCanvas, ctx, uiLayer, logicalWidth, logicalHeight, lastTime, gameState, timeLeft, scoreModifier, balls, particles, players, currentPlayerName, currentPlayerNum, sounds, soundsInitialized, isShaking, SLOTS, BALL_RADIUS, sumDisplayEl, btnPlusEl, btnMinusEl, timerDisplayEl, ctrlPanelEl;
var init_app = __esm({
  "public/20260422113142_d9768f34-3cde-46a7-9d76-107a6f2d20b2/app.ts"() {
    init_appHelper();
