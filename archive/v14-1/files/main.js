...
function setupPlayUI() {
  ...
  uiLayer.appendChild(sumDisplayEl);
...
function render3() {
  ...
  if (gameState === "PLAY" || gameState === "TIMEUP") {
    ...
    let boxW = 860;
    let boxH = 220;
    let boxX = logicalWidth / 2 - boxW / 2;
  ...
  if (slotCount === 3) {
    if (btnPlusEl && btnMinusEl && sumDisplayEl && ctrlPanelEl) {
      ...
      if (diff <= 1) {
        if (!isShaking) {
          isShaking = true;
          gsapWithCSS.to(sumDisplayEl, { rotation: 2, yoyo: true, repeat: -1, duration: 0.05 });
        }
      } else {
        if (isShaking) {
          isShaking = false;
          gsapWithCSS.killTweensOf(sumDisplayEl);
          gsapWithCSS.set(sumDisplayEl, { rotation: 0 });
        }
      }
    }
  } else {
