    ...
    if (btnPlusEl && btnMinusEl && sumDisplayEl && ctrlPanelEl) {
      ...
      if (diff <= 1) {
        if (!isShaking) {
          isShaking = true;
          if (sumContainerEl) {
            gsapWithCSS.fromTo(sumContainerEl, { x: -10 }, { x: 10, yoyo: true, repeat: -1, duration: 0.05 });
          }
        }
      } else {
        if (isShaking) {
          isShaking = false;
          if (sumContainerEl) {
            gsapWithCSS.killTweensOf(sumContainerEl);
            gsapWithCSS.set(sumContainerEl, { x: 0 });
          }
        }
      }
    }
