...
// public/20260422113142_d9768f34-3cde-46a7-9d76-107a6f2d20b2/appHelper.ts
var AppHelper;
...
var init_gsap_core = __esm({
  "node_modules/gsap/gsap-core.js"() {
    ...
    Animation = /* @__PURE__ */ (function() {
      ...
      _proto.globalTime = function globalTime2(rawTime) {
...
// public/20260422113142_d9768f34-3cde-46a7-9d76-107a6f2d20b2/app.ts
function showPlayerSelectionPopup() {
  ...
  gsapWithCSS.from(container, { opacity: 0, scale: 1.1, duration: 0.4, ease: "back.out(1.2)" });
}
function showRanking(isFinal) {
  ...
  let oldYPositions = /* @__PURE__ */ new Map();
  const createItem = (p, idx, isTop) => {
    ...
    let rankLabel = isTop ? "\u{1F451} 1\uC704." : `${idx + 1}\uC704.`;
  ...
  let finalDivs = /* @__PURE__ */ new Map();
  ...
  if (isFinal) {
    let fireConfettiCycle = function() {
      if (gameState !== "RESULT") return;
      let end = Date.now() + 3 * 1e3;
      (function frame() {
        if (gameState !== "RESULT") return;
        ...
        if (Date.now() < end) {
          ...
          setTimeout(fireConfettiCycle, 2e3);
        }
      })();
    };
...
function renderPlayerInputs() {
  ...
  for (let i = 0; i < selectedRoundCount; i++) {
    ...
    inpL.value = i < savedL.length ? savedL[i] : `L-${i + 1}\uBC88`;
    ...
    inpR.value = i < savedR.length ? savedR[i] : `R-${i + 1}\uBC88`;
    listR.appendChild(inpR);
  }
}
function updateRoundCount(delta) {
...
function onStartClick() {
  if (!soundsInitialized) {
    assetList.sounds.forEach((snd) => {
      sounds[snd.id] = new import_howler.Howl({
        src: [snd.file_path],
  ...
  playerNamesR = [];
  for (let i = 0; i < selectedRoundCount; i++) {
    let inpL = document.getElementById(`playerInpL_${i}`);
    let inpR = document.getElementById(`playerInpR_${i}`);
...
function createControlButton(label, team, handler) {
  let isPlus = label === "+";
  let bg = isPlus ? "radial-gradient(circle at 30% 30%, #FFCC66, #FFA500 60%, #994C00)" : "radial-gradient(circle at 30% 30%, #4D4DFF, #0000FF 60%, #000033)";
...
function createStopButton(team, handler) {
  return AppHelper.createUIElement(
    "button",
    `btnStop${team}`,
    {
...
function onModifierClick(team, val, btn) {
  ...
  if (btn) {
    gsapWithCSS.killTweensOf(btn);
    gsapWithCSS.fromTo(btn, { scale: 1 }, { scale: 0.8, yoyo: true, repeat: 1, duration: 0.1 });
...
function showFloatingText(val, x, y) {
  let startY = val < 0 ? y - 100 : y;
  ...
  let targetY = val > 0 ? "-=150" : "+=150";
  gsapWithCSS.to(txt, { y: targetY, opacity: 0, duration: 1, ease: "power1.out", onComplete: () => txt.remove() });
}
function calculateSum() {
  ...
  if (!leftStopped) {
    if (countL === 3) {
      ...
      if (Math.abs(totalL - appData.targetScore) <= appData.shakeTolerance && gameState === "PLAY") {
        if (!isShakingL) {
          isShakingL = true;
          gsapWithCSS.fromTo(shakeOffsetL, { x: -2.6 }, { x: 2.6, yoyo: true, repeat: -1, duration: 0.05 });
        }
      } else {
        isShakingL = false;
        gsapWithCSS.killTweensOf(shakeOffsetL);
        shakeOffsetL.x = 0;
      }
    } else {
      if (btnPlusL && btnMinusL && ctrlPanelL) {
  ...
  if (!rightStopped) {
    if (countR === 3) {
      ...
      if (Math.abs(totalR - appData.targetScore) <= appData.shakeTolerance && gameState === "PLAY") {
        if (!isShakingR) {
          isShakingR = true;
          gsapWithCSS.fromTo(shakeOffsetR, { x: -2.6 }, { x: 2.6, yoyo: true, repeat: -1, duration: 0.05 });
        }
      } else {
        isShakingR = false;
        gsapWithCSS.killTweensOf(shakeOffsetR);
        shakeOffsetR.x = 0;
      }
    } else {
      if (btnPlusR && btnMinusR && ctrlPanelR) {
...
function timeUp(isStopAll = false) {
  if (gameState !== "PLAY") return;
  gameState = "TIMEUP";
  sounds["bgm"]?.stop();
  sounds["timeup"]?.play();
  ...
  gsapWithCSS.killTweensOf(shakeOffsetR);
  shakeOffsetR.x = 0;
  let sumL = balls.filter((b) => b.team === "L" && b.state === "SLOT").reduce((acc, b) => acc + b.num, 0) + scoreModifierL;
  let sumR = balls.filter((b) => b.team === "R" && b.state === "SLOT").reduce((acc, b) => acc + b.num, 0) + scoreModifierR;
  players.push({
    name: activePlayerL,
    score: sumL,
  ...
  let tUp = AppHelper.createUIElement(
    ...
    "timeUpTxt",
    {
      ...
      fontFamily: "sans-serif",
      color: "#FF0000",
      textShadow: "0 0 60px rgba(255,255,255,0.8), 0 0 20px #FFF",
  ...
  gsapWithCSS.fromTo(tUp, { scale: 0, opacity: 0 }, { scale: 1, opacity: 1, duration: 0.6, ease: "back.out(1.5)" });
  setTimeout(() => {
    gsapWithCSS.to(tUp, {
      scale: 0,
      opacity: 0,
...
function drawSlotBox(cx, cy, isFull, offsetX) {
  let boxW = 460;
  let boxH = 160;
...
var import_howler, activePlayerR, activePlayerL, remainingPlayersR, remainingPlayersL, logicalWidth, logicalHeight, appData, textData, assetList, appCanvas, ctx, uiLayer, lastTime, globalTime, gameState, timeLeft, soundsInitialized, sounds, selectedRoundCount, currentRoundIndex, playerNamesL, playerNamesR, players, leftStopped, rightStopped, leftTimeLeft, rightTimeLeft, scoreModifierL, scoreModifierR, Ball, balls, Particle, particles, BALL_RADIUS, RingParticle, ringParticles, shakeOffsetL, shakeOffsetR, isShakingL, isShakingR, SLOTS_L, SLOTS_R, sumDisplayL, btnPlusL, btnMinusL, btnStopL, ctrlPanelL, sumDisplayR, btnPlusR, btnMinusR, btnStopR, ctrlPanelR, timerDisplayEl;
var init_app = __esm({
  "public/20260422113142_d9768f34-3cde-46a7-9d76-107a6f2d20b2/app.ts"() {
    init_appHelper();
...
// public/20260422113142_d9768f34-3cde-46a7-9d76-107a6f2d20b2/main.ts
var require_main = __commonJS({
  "public/20260422113142_d9768f34-3cde-46a7-9d76-107a6f2d20b2/main.ts"() {
    init_app();
    init_appHelper();
    var logicalWidth2 = 0;
    var logicalHeight2 = 0;
      ...
      if (!isCanvasLayoutUpdating) {
        window.requestAnimationFrame(() => {
          ...
          if (appCanvas2.width !== 1 && appCanvas2.height !== 1) {
            if (logicalWidth2 === 0 && logicalHeight2 === 0) {
              logicalWidth2 = appCanvas2.width;
              logicalHeight2 = appCanvas2.height;
            }
            ...
            const uiLayerScale = appCanvas2.width / logicalWidth2;
            ;
            uiLayer2.style.position = "absolute";
            uiLayer2.style.width = logicalWidth2 + "px";
            uiLayer2.style.height = logicalHeight2 + "px";
